...
@section('page_title', __('Редактирование'))
@section('page_excerpt', __('Редактирование'))
        ...
        <div class="nk-block-head nk-block-head-sm">
            <div class="nk-block-between g-3">
                <div class="nk-block-head-content">
                    <h3 class="nk-block-title page-title">{{ __('Редактирование') }} </h3>
                </div>
                <div class="nk-block-head-content">
            ...
            <div class="card card-bordered">
                <div class="card-aside-wrap">
                    <div class="card-content">
                        <form id="base_form" method="POST" enctype="multipart/form-data" action="{{ route('source.update', $item->id) }}">
                            @method('PATCH')
                            @csrf
                                                        ...
                                                        <div class="form-control-wrap">
                                                            <select name="key" class="form-select">
                                                                @foreach($key_list as $key)
                                                                    <option value="{{ $key }}" @if($item->key == $key) selected @endif>{{ $key }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                    </div>
