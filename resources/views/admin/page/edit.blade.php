...
@section('page_title', __('Редактирование страницы'))
@section('page_excerpt', __('Редактирование страницы'))
        ...
        <div class="nk-block-head nk-block-head-sm">
            <div class="nk-block-between g-3">
                <div class="nk-block-head-content">
                    <h3 class="nk-block-title page-title">{{ __('Редактирование страницы') }} </h3>
                </div>
                <div class="nk-block-head-content d-flex">
                    @if($item->published_at)
                        <form method="POST" action="{{ route('admin.page.not_publish', $item->id) }}">
                            @csrf
                            <button type="submit" class="btn btn-sm btn-secondary mr-3" data-toggle="tooltip" data-placement="top" title="{{ __('Снять с публикации') }}"><em class="icon ni ni-eye-off-fill"></em></button>
                        </form>
                    @else
                        <form method="POST" action="{{ route('admin.page.publish', $item->id) }}">
                            @csrf
                            <button type="submit" class="btn btn-sm btn-success" data-toggle="tooltip" data-placement="top" title="{{ __('Опубликовать') }}"><em class="icon ni ni-upload-cloud"></em></button>
                        </form>
                    @endif
            ...
            <div class="card card-bordered">
                <div class="card-aside-wrap">
                    <div class="card-content">
                        <form method="POST" action="{{ route('admin.page.update', $item->id) }}" enctype="multipart/form-data">
                            @method('PATCH')
                            @csrf
